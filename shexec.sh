...
PKG=(ssh sshpass yad sendemail cron)
...
function settings()
{
	...
	if OPTCONF="$(yad --title="Configurações" \
				...
				--field='Visualizar/Adicionar!gtk-index!Visualizar/Editar agendamentos':BTN "./shexec --schedule" \
				--field='':LBL '' \
				--field='':LBL '' \
				--field='':LBL '')"; then
...
{	
	...
	OPT=$(yad --form  \
				...
				--field="Lista de hosts":BTN "./shexec win_hosts" \
				...
				--field='Novo!gtk-new!Novo perfil.':BTN "@./shexec profile new" \
				--field='Salvar!gtk-save!Salva as configurações atuais.':BTN "./shexec profile save '%3|%4|%5|%6|%13|%14|%15|%16|%17|%23|%26|%27|%29|%33|%34|%35|%36|%37'" \
				--field='Carregar!gtk-open!Carrega as configurações do perfil salvo.':BTN "@./shexec profile load")
